  ...
  try {
    const order = {
      ...
      application_context: {
        ...
        failure_url: `https://cartastarotecuador.com/welcome`,
        cancel_url: `https://cartastarotecuador.com/welcome`,
      },
    };
...
export const captureOrder = async (req, res) => {
  ...
  try {
    ...
    if (response.data.status === "COMPLETED") {
      ...
      return res.redirect(
        `https://cartastarotecuador.com/descripcion-cartas?status=NOT_COMPLETED&token=${rejectToken}`
      );
    }
  } catch (error) {
    if (error.response) {
      console.error('Error de PayPal:', error.response.data);
      return res.redirect(`https://cartastarotecuador.com/descripcion-cartas?status=ERROR`);
    }
